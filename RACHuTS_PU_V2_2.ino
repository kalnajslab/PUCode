...
/* Profile Data Buffer */
uint16_t ProfileData[TM_RECORD_LENGTH][TM_BUFFER_LENGTH];  //120.4KB
...
uint16_t TSENData[TSEN_RECORD_LENGTH][TSEN_BUFFER_LENGTH];// 7.2 KBytes 
...
uint16_t FLASH_intT;
uint16_t FLASH_pmtV;
uint16_t FLASH_lampI;
uint16_t FLASH_lampV;
uint16_t FLASH_lampT;
uint16_t FLASH_V;
uint16_t FLASH_T;
   ...
   */
   ...
   while(!checkForSerial())
   {
     ...
     if (millis() > startTime)
     {
      ...
      DEBUG_SERIAL.printf("#67,%d,%5.2f,%5.2f,%5.2f,%5.2f,%5.2f,%5.2f,%d,%d,%d\r\n", millis()/1000,HTR1_Therm, HTR2_Therm, PUMP_Therm, V_Battery, V_Input, I_Charge,Heater1Status, Heater2Status,TSENData[1][TSENindx]);
     }
     delay(1);  
   }
  ...
  while((FLASH_float_T < FLASH_T) || (HTR1_Therm < Heater1Setpoint) || (HTR2_Therm < Heater2Setpoint) )
   {
     ...
     if (millis() > startTime)
     {
      ...
      setHeaters();
      TSENindx = getTSEN(TSENindx);
      //GONDOLA_SERIAL.printf("#68,%d,%5.2f,%5.2f,%5.2f,%5.2f,%5.2f,%5.2f,%d,%d,%d,%5.2f,%5.2f\r\n",millis()/1000, HTR1_Therm, HTR2_Therm, V_Battery, V_Input, I_Charge, I_Flash, Heater1Status, Heater2Status,TSENData[1][TSENindx], FLASH_float_T, FLASH_float_lampT );
  ...
  while(dataRecords*ProfileMovingDataRate < ProfileDownTime)
   {
     ...
     if (millis() > startTime)
     {
      ...
      DEBUG_SERIAL.printf("#70,%d,%5.2f,%5.2f,%5.2f,%5.2f,%5.2f,%5.2f,%d,%d,%d,%5.2f,%5.2f,%d,%d,%d,%d,%5.2f\r\n",millis()/1000, HTR1_Therm, HTR2_Therm, V_Battery, V_Input,I_Charge, I_Flash, Heater1Status, Heater2Status,TSENData[1][TSENindx], FLASH_float_T, FLASH_float_lampT,FLASH_fluor, FLASH_bkg, OPC_300, OPC_500, PUMP_Therm );
      dataRecords++;
     }
   }
  ...
  while(dataRecords*ProfileDwellDataRate < ProfileDwellTime)
   {
     ...
     if (millis() > startTime)
     {
      ...
      TSENindx = getTSEN(TSENindx);
      TMRecordIndx = SaveRecord(TMRecordIndx); //writes a data record to the TM array
      DEBUG_SERIAL.printf("#71,%d,%5.2f,%5.2f,%5.2f,%5.2f,%5.2f,%5.2f,%d,%d,%d,%5.2f,%5.2f,%d,%d,%d,%d,%5.2f\r\n",millis()/1000, HTR1_Therm, HTR2_Therm, V_Battery, V_Input,I_Charge, I_Flash, Heater1Status, Heater2Status,TSENData[1][TSENindx], FLASH_float_T, FLASH_float_lampT,FLASH_fluor, FLASH_bkg, OPC_300, OPC_500, PUMP_Therm );
      dataRecords++;
     }
     delay(1);  
   }
  ...
  while(dataRecords*ProfileMovingDataRate < ProfileUpTime)
   {
     ...
     if (millis() > startTime)
     {
      ...
      TSENindx = getTSEN(TSENindx);
      TMRecordIndx = SaveRecord(TMRecordIndx); //writes a data record to the TM array
      DEBUG_SERIAL.printf("#72,%d,%5.2f,%5.2f,%5.2f,%5.2f,%5.2f,%5.2f,%d,%d,%d,%5.2f,%5.2f,%d,%d,%d,%d,%5.2f\r\n",millis()/1000, HTR1_Therm, HTR2_Therm, V_Battery, V_Input,I_Charge, I_Flash, Heater1Status, Heater2Status,TSENData[1][TSENindx], FLASH_float_T, FLASH_float_lampT,FLASH_fluor, FLASH_bkg, OPC_300, OPC_500, PUMP_Therm );
      dataRecords++;
     }
   }
...
int SaveRecord(int dataindx)
{
    ...
    if(dataindx = 0) //first line of array is header line
    {
        ...
        ProfileData[0][0] = (uint16_t) (GPSStartTime >> 16);  
        ProfileData[1][0] = (uint16_t) (GPSStartTime);
        ...
        *(float*)(bytes) = single;  // convert float to bytes
        ProfileData[2][0] = (uint16_t)((bytes[3] << 8) + bytes[2]); //high bytes (big endian)
        ProfileData[3][0] = (uint16_t)((bytes[1] << 8) + bytes[0]); //low bytes (big endian)
        
        GPSStartLon = gps.location.lng();
        single = (float)GPSStartLon;  //convert double to single float
        *(float*)(bytes) = single;  // convert float to bytes
        ProfileData[4][0] = (uint16_t)((bytes[3] << 8) + bytes[2]); //high bytes (big endian)
        ProfileData[5][0] = (uint16_t)((bytes[1] << 8) + bytes[0]); //low bytes (big endian)
        
        ProfileData[6][0] = (uint16_t) (ZephyrAlt); //Altitude in meters
        
        *(float*)(bytes) = ZephyrLat;  // convert float to bytes
        ProfileData[7][0] = (uint16_t)((bytes[3] << 8) + bytes[2]); //high bytes (big endian)
        ProfileData[8][0] = (uint16_t)((bytes[1] << 8) + bytes[0]); //low bytes (big endian)

        *(float*)(bytes) = ZephyrLon;  // convert float to bytes
        ProfileData[9][0] = (uint16_t)((bytes[3] << 8) + bytes[2]); //high bytes (big endian)
        ProfileData[10][0] = (uint16_t)((bytes[1] << 8) + bytes[0]); //low bytes (big endian)

        ProfileData[11][0] = (uint16_t)(V_3v3*1000);  //Voltage in mV
        ProfileData[12][0] = (uint16_t)(V_Input*1000);  //Voltage in mV
        ProfileData[13][0] = (uint16_t)(I_Charge*1000);  //Current in mA
    }

    else{
        ProfileData[0][dataindx] = (uint16_t)(now() - GPSStartTime); // Elapsed time in seconds
        ProfileData[1][dataindx] = (uint16_t)(gps.location.lat() - GPSStartLat)*10000; //difference in lat 0.0001 degrees
        ProfileData[2][dataindx] = (uint16_t)(gps.location.lng() - GPSStartLon)*10000; //difference in lon 0.0001 degrees
        ProfileData[3][dataindx] = (uint16_t)(gps.altitude.meters());  //Altitude in meters
        ProfileData[4][dataindx] = (uint16_t)(FLASH_fluor);  //Flash-B FLouresence signal
        ProfileData[5][dataindx] = (uint16_t)(FLASH_bkg);  //Flash-B Background signal
        ProfileData[6][dataindx] = (uint16_t)(OPC_300);  //ROPC 300nm Channel
        ProfileData[7][dataindx] = (uint16_t)(TSENData[1][TSENindx-1]);  //TSEN variable 1
        ProfileData[8][dataindx] = (uint16_t)(TSENData[2][TSENindx-1]);  //TSEN variable 2
        ProfileData[9][dataindx] = (uint16_t)(TSENData[3][TSENindx-1]);  //TSEN variable 3
        ProfileData[10][dataindx] = (uint16_t)((HK_enum << 8) + Heater2Status*2 + Heater1Status); //Enum index and heater on/off bits
        switch (HK_enum){
            case 0:
                ProfileData[11][dataindx] =(uint16_t)((FLASH_float_T + 273.15) * 100); //Flash internal T in K at 0.01C precision
                ProfileData[12][dataindx] = (uint16_t)(OPC_500); //ROPC 500nm Channel
                ProfileData[13][dataindx] = (uint16_t)(V_Battery*1000); //Battery V in mV
                break;
            case 1:
                ProfileData[11][dataindx] =(uint16_t)(FLASH_pmtV * 1000); //Flash PMT Voltage in mV
                ProfileData[12][dataindx] = (uint16_t)(OPC_700); //ROPC 700nm Channel
                ProfileData[13][dataindx] = (uint16_t)(V_Pump*1000); //Pump V in mV
                break;
            case 2:
                ProfileData[11][dataindx] = (uint16_t)(FLASH_lampI * 1000); //Flash lamp current in mA
                ProfileData[12][dataindx] = (uint16_t)(OPC_1000); //ROPC 1000nm Channel
                ProfileData[13][dataindx] = (uint16_t)(I_Tsen*1000); //TSEN Current in mA
                break;
            case 3:
                ProfileData[11][dataindx] = (uint16_t)(FLASH_lampV * 1000); //Flash lamp voltage in mV
                ProfileData[12][dataindx] = (uint16_t)(OPC_2000); //ROPC 2000nm Channel
                ProfileData[13][dataindx] = (uint16_t)(I_Flash*1000); //FLASH Current in mA
                break;
            case 4:
                ProfileData[11][dataindx] = (uint16_t)((FLASH_lampT + 273.15) * 100); //Flash lamp T in K at 0.01C precision
                ProfileData[12][dataindx] = (uint16_t)(OPC_3000); //ROPC 3000nm Channel
                ProfileData[13][dataindx] = (uint16_t)(I_Opc*1000); //ROPC Current in mA
                break;
            case 5:
                ProfileData[11][dataindx] = (uint16_t)(FLASH_V * 1000); //Flash Voltage in mV
                ProfileData[12][dataindx] = (uint16_t)(OPC_5000); //ROPC 5000nm Channel
                ProfileData[13][dataindx] = (uint16_t)((PUMP_Therm + 273.15)*100); //ROPC Pump T in K at 0.01C precision
                break;
            case 6:
                ProfileData[11][dataindx] = (uint16_t)((FLASH_T + 273.15) * 100); //Flash T in K at 0.01C precision
                ProfileData[12][dataindx] = (uint16_t)(OPC_10000); //ROPC 10000nm Channel
                ProfileData[13][dataindx] = (uint16_t)((HTR1_Therm + 273.15)*100); //Heater 1 T in K at 0.01C precision
                break;
            case 7:
                ProfileData[11][dataindx] = (uint16_t) 0; //Currently unused
                ProfileData[12][dataindx] = (uint16_t) 0; //Currently unused
                ProfileData[13][dataindx] = (uint16_t)((HTR2_Therm + 273.15)*100); //Heater 2 T in K at 0.01C precision
                break;
        }
        HK_enum++;
        if (HK_enum > 7)
  ...
  if (dataIndex == 0) //if this is the first sample in the TM packet write the header
    {
        ...
        TSENData[0][0] = (uint16_t) (TSENDataStartTime >> 16);  
        TSENData[1][0] = (uint16_t) (TSENDataStartTime);
        TSENData[2][0] = (uint16_t) (I_Tsen * 1000); //TSEN Current
        TSENData[3][0] = (uint16_t) ((HTR1_Therm+ 273.15)*100);  //Temperature near TSEN
        dataIndex++;
    }
  ...
  //delay(1); //make sure there is time for TSEN to respond
   ...
   if(TSEN_Char == '\r') //got a EOL from TSEN
   {
      if ( TSEN_Buff.startsWith("#")) //String starts with the right char
      {
        ...
        TSENData[0][dataIndex] = (uint16_t)(now() - TSENDataStartTime);
        TSEN_Buff.toCharArray(tsen_buff,32);
        TSENData[1][dataIndex] = (uint16_t)strtoul(tsen_buff+1,&pEnd,16);
        TSENData[2][dataIndex] = (uint16_t)strtoul(pEnd,&pEnd,16);
        TSENData[3][dataIndex] = (uint16_t)strtoul(pEnd,NULL,16);
 ...
 */
 ...
 { 
    ...
    ser.AssignBinaryTXBuffer((uint8_t *) ProfileData + TMRecordOffset, remainingRecords * TM_RECORD_LENGTH * 2, remainingRecords * TM_RECORD_LENGTH * 2);
    ser.TX_Bin(PU_PROFILE_RECORD);
 } 
 //send the full number of records if available
 else{
    ser.AssignBinaryTXBuffer((uint8_t *) ProfileData + TMRecordOffset, TMRecordsToSend * TM_RECORD_LENGTH * 2,TMRecordsToSend * TM_RECORD_LENGTH *2);
    ser.TX_Bin(PU_PROFILE_RECORD);
 }
 ...
 while(millis() < TimeOut)
    ...
    if(ser.ack_id == PU_SEND_PROFILE_RECORD && ser.ack_value == true)
        {
 ...
 */
 ...
 { 
    ...
    ser.AssignBinaryTXBuffer((uint8_t *) TSENData + TSENRecordOffset, remainingRecords * TSEN_RECORD_LENGTH * 2, remainingRecords * TSEN_RECORD_LENGTH * 2);
    ser.TX_Bin(PU_TSEN_RECORD);
 } 
 //send the full number of records if available
 else{
    ser.AssignBinaryTXBuffer((uint8_t *) TSENData + TSENRecordOffset, TSENRecordsToSend * TSEN_RECORD_LENGTH * 2,TSENRecordsToSend * TSEN_RECORD_LENGTH *2);
    ser.TX_Bin(PU_TSEN_RECORD);
 }
 ...
 while(millis() < TimeOut)
    ...
    if(ser.ack_id == PU_SEND_TSEN_RECORD && ser.ack_value == true)
        {
...
void scanAnalog()
{
  ...
  I_Charge = analogRead(I_CHARGE) * 0.00073 * 2.4547;
         ...
         case ASCII_MESSAGE:
            ...
            switch (ser.ascii_rx.msg_id){
                ...
                case PU_SEND_PROFILE_RECORD:
                    sendTM();
                    DEBUG_SERIAL.println("Received PU_SEND_PROFILE_RECORD");
                    return false;
                case PU_SEND_TSEN_RECORD:
                    sendTSEN();
                    DEBUG_SERIAL.println("Received PU_SEND_TSEN_RECORD");
                    return false;
                case PU_RESET:
                    ser.TX_Ack(PU_RESET,true);
                ...
                case PU_SET_HEATERS:
                    ...
                    ser.TX_Ack(PU_SET_HEATERS,tmp1);
                    if(tmp1)
                    {
...
bool parseFLASH()
{
  ...
  if ( FLASH_Buff.startsWith(header))
  {
    ...
    //DEBUG_SERIAL.print(FLASH_lampT);
